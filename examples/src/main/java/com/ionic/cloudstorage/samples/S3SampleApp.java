...
public class S3SampleApp {
    ...
    public static final int MAX_AWS_SDK_CHUNKS = 10000; // AWS S3 limits multipart uploads to 10000
                                                        // parts
        ...
        try {
            ...
            for (int i = 1; i <= Math.min(MAX_AWS_SDK_CHUNKS, totalChunksInt); i++) {
                ...
                UploadPartRequest uploadRequest = new UploadPartRequest().withBucketName(bucketName)
                        .withKey(objectKey).withUploadId(res.getUploadId()).withPartNumber(i)
                        .withFileOffset(filePosition).withFile(file).withPartSize(partSizeBytes)
                        .withLastPart(isLast);
